...
class ModuleUtilsMixin:
    ...
    def get_extended_attention_mask(self, attention_mask: Tensor,
                                    input_shape: tuple, device: device):
    ...
    def _convert_head_mask_to_5d(self, head_mask, num_hidden_layers):
        ...
        head_mask = head_mask.to(
            dtype=self.dtype)  # switch to fload if need + fp16 compatibility
        return head_mask
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def prune_heads(self, heads_to_prune):
        ...
        for layer, heads in heads_to_prune.items():
            union_heads = set(self.config.pruned_heads.get(layer,
                                                           [])) | set(heads)
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            # Get the mask tensor based on the previous tokens and apply it
            if mask_function is not None:
                mask = mask_function(input_ids)
                next_token_logits += mask

            # repetition penalty from CTRL paper (https://arxiv.org/abs/1909.05858)
            if repetition_penalty != 1.0:
                self.enforce_repetition_penalty_(next_token_logits, batch_size,
                                                 1, input_ids,
                                                 repetition_penalty)
        ...
        for hypo_idx, hypo in enumerate(input_ids):
            decoded[hypo_idx, :
                    sent_lengths[hypo_idx]] = hypo[:sent_lengths[hypo_idx]]
        ...
        while cur_len < max_length:
            ...
            if no_repeat_ngram_size > 0:
                ...
                banned_batch_tokens = calc_banned_ngram_tokens(
                    input_ids, num_batch_hypotheses, no_repeat_ngram_size,
                    cur_len)
...
def apply_chunking_to_forward(chunk_size: int, chunk_dim: int,
    ...
    assert len(
        input_tensors) > 0, "{} has to be a tuple/list of tensors".format(
            input_tensors)
